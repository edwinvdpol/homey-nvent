  ...
  */
  ...
  async sync() {
    ...
    try {
      ...
      // Keep the device available, but surface API-reported offline.
  ...
  async syncWarning(data) {
    if (data.alarm_connectivity === true) {
      return this.setWarning(this.homey.__('error.offline'));
    }
